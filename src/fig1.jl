...
gene = Gene("FIGNL1", gencode, expr, expri, cov, 1e6, geno, "both") # PHLDB1, MRPL43
qtls, mega = runqtl(gene)
vc = CSV.read(joinpath(@__DIR__, "../results/multivariate/$(gene.gene_id).tsv"), delim = "\t", DataFrame)
isoforms = unique(vc.id)
...
function parse_mul(result, transcript_id)
    ...
    for i in 1:nrow(result)
        ...
        if ind1 != ind2
            for j in 1:3
                Σ̂[1][ind1, ind2] = Σ̂[1][ind2, ind1] = 3 * result[i, "var/covar_cis_reml"]
                Σ̂[2][ind1, ind2] = Σ̂[2][ind2, ind1] = 3 * result[i, "var/covar_trans_reml"]
                Σ̂[3][ind1, ind2] = Σ̂[3][ind2, ind1] = 3 * result[i, "var/covar_res_reml"]
            end
        else
            for j in 1:3
                Σ̂[1][ind1, ind2] = Σ̂[1][ind2, ind1] = result[i, "var/covar_cis_reml"]
                Σ̂[2][ind1, ind2] = Σ̂[2][ind2, ind1] = result[i, "var/covar_trans_reml"]
                Σ̂[3][ind1, ind2] = Σ̂[3][ind2, ind1] = result[i, "var/covar_res_reml"]
            end
        end
    end
    Σ̂
end
Σ̂ = parse_mul(vc, isoforms_sorted)
Σ̂p = Σ̂[1] + Σ̂[2] + Σ̂[3]
Σ̂ ./= maximum(vec(Σ̂p))
Σ̂p ./= maximum(vec(Σ̂p))
...
begin
    ...
    xlims!(axs[2], range1 - 0.5e4, range2 + 0.5e4)
    ylims!(axs[2], 0.875 - (d + 1) * 0.125, 1.175)
    rowsize!(gs[1], 1, 18 * (d + 2) * 0.125 / 0.5)
    colsize!(gs[1], 2, 230)
    snps = filter(x -> range1 - 0.5e4 < x < range2 + 0.5e4, gene.snpinfo.position)
    barplot!(axs[2], snps,
        fill(0.875 - (d - 0.75) * 0.125, length(snps)), fillto = 0.875 - d * 0.125,
        color = "#389826")
    hidedecorations!(axs[2])
    Label(gs[1][1, 2, Top()], "1. Define cis- and trans-SNPs", textsize = 6)
    ...
    Label(gs[2][1, 1:4, Top()], "2. Fit multivariate variance components model", textsize = 6)
    rowgap!(g2, 3)
    colgap!(g2, 12.5)
    ...
    Label(gs[3][1, 1, Top()], "3. Run cis-eQTL analysis", textsize = 6)
    rowsize!(gs[3], 1, 0.1)
    rowgap!(gs[3], 1, 3)
    hidespines!(axs[10])
    hidedecorations!(axs[10])
    qtls_subset2 = filter(x -> range1 - 0.45e5 < x.BP < range2 + 0.45e5, qtls_subset)
    ...
    save("figure1-$(gene.gene_name).png", f, px_per_unit = 4)
end
