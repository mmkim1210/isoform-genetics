...
let
    ...
    Label(g1[0, 1:2], text = rich("Distribution of ", rich("h", font = :italic), "²", subscript("SNP"), " estimates"), fontsize = 8, tellwidth = false)
    Label(g1[0, 3], rich("Dist. of ", rich("r", font = :italic), subscript("g"), " estimates"), fontsize = 8, tellwidth = false)
    Label(g1[1, 1, Top()], "Genes", fontsize = 6)
    Label(g1[1, 2, Top()], "Isoforms", fontsize = 6)
    Label(g1[1, 3, Top()], "Isoforms", fontsize = 6)
    ...
    rowsize!(g1, 1, Aspect(3, 1))
    Label(g1[0, 1:2, Left()], "a", fontsize = 12, font = "Arial bold", padding = (0, 5, 0, 0), halign = :right)
    Label(g1[0, 3, Left()], "d", fontsize = 12, font = "Arial bold", padding = (0, 5, 0, 0), halign = :right)
    colgap!(g1, 2.5)
    rowgap!(g1, 0)
    ...
    [ablines!(axs2[j], 0, 1, color = ("#CB3C33", 1), linewidth = 0.5) for j in 1:3]
    for j in 1:3
        data = DataFrame(x = df_uni[:, 1 + j ], y = df_h2[:, 1 + j])
        model = lm(@formula(y ~ x), data)
        ablines!(axs2[j], coef(model)[1], coef(model)[2], color = :gold, linewidth = 0.5)
    end
    ...
    Label(g2[0, 1:3], text = "Comparison of variance components estimates", fontsize = 8, tellwidth = false)
    Label(g2[1, 1, Top()], rich(rich("Cis", font = :italic), "-genetic variance"), fontsize = 6)
    Label(g2[1, 2, Top()], rich(rich("Trans", font = :italic), "-genetic variance"), fontsize = 6)
    Label(g2[1, 3, Top()], "Residual variance", fontsize = 6)
    Label(g2[0, 1:3, Left()], "b", fontsize = 12, font = "Arial bold", padding = (0, 5, 0, 0), halign = :right)
    Label(g2[2, 1:3], text = "Univariate estimates", fontsize = 6)
    Label(g2[1, 0], text = "Multivariate estimates", fontsize = 6, rotation = π / 2, tellheight = false)
    colgap!(g2, 2.5)
    rowgap!(g2, 0)
    ...
    [scatter!(axs3[j], df_uni[:, 7 + j], df_h2[:, 7 + j], 
        color = (colors[j], 0.5), markersize = 2) for j in 1:3]
    [ablines!(axs3[j], 0, 1, color = ("#CB3C33", 1), linewidth = 0.5) for j in 1:3]
    for j in 1:3
        data = DataFrame(x = df_uni[:, 7 + j ], y = df_h2[:, 7 + j])
        model = lm(@formula(y ~ x), data)
        ablines!(axs3[j], coef(model)[1], coef(model)[2], color = :gold, linewidth = 0.5)
    end
    ...
    Label(g3[0, 1:3], text = rich("Comparison of ", rich("h", font = :italic), "²", subscript("SNP"), " estimates"), fontsize = 8, tellwidth = false)
    Label(g3[1, 1, Top()], rich(rich("h", font = :italic), "²", subscript("cis")), fontsize = 6)
    Label(g3[1, 2, Top()], rich(rich("h", font = :italic), "²", subscript("trans")), fontsize = 6)
    Label(g3[1, 3, Top()], rich(rich("h", font = :italic), "²", subscript("SNP")), fontsize = 6)
    Label(g3[0, 1:3, Left()], "c", fontsize = 12, font = "Arial bold", padding = (0, 5, 0, 0), halign = :right)
    Label(g3[2, 1:3], text = "Univariate estimates", fontsize = 6)
    Label(g3[1, 0], text = "Multivariate estimates", fontsize = 6, rotation = π / 2, tellheight = false)
    colgap!(g3, 2.5)
    rowgap!(g3, 0)
    ...
    [scatter!(axs4[j], df_rg_bi[:, 2 + j], df_rg_mul[:, 2 + j], 
        color = (colors[j], 0.5), markersize = 2) for j in 1:3]
    [ablines!(axs4[j], 0, 1, color = ("#CB3C33", 1), linewidth = 0.5) for j in 1:3]
    for j in 1:3
        data = DataFrame(x = df_rg_bi[:, 2 + j], y = df_rg_mul[:, 2 + j])
        model = lm(@formula(y ~ x), data)
        ablines!(axs4[j], coef(model)[1], coef(model)[2], color = :gold, linewidth = 0.5)
    end
    ...
    Label(g4[0, 1:3], text = "Comparison of variance components estimates", fontsize = 8, tellwidth = false)
    Label(g4[1, 1, Top()], rich(rich("Cis", font = :italic), "-genetic covariance"), fontsize = 6)
    Label(g4[1, 2, Top()], rich(rich("Trans", font = :italic), "-genetic covariance"), fontsize = 6)
    Label(g4[1, 3, Top()], "Residual covariance", fontsize = 6)
    Label(g4[0, 1:3, Left()], "e", fontsize = 12, font = "Arial bold", padding = (0, 5, 0, 0), halign = :right)
    Label(g4[2, 1:3], text = "Pairwise bivariate estimates", fontsize = 6)
    Label(g4[1, 0], text = "Multivariate estimates", fontsize = 6, rotation = π / 2, tellheight = false)
    colgap!(g4, 2.5)
    rowgap!(g4, 0)
    ...
    [scatter!(axs5[j], df_rg_bi[:, 8 + j], df_rg_mul[:, 8 + j], 
        color = (colors[j], 0.5), markersize = 2) for j in 1:3]
    [ablines!(axs5[j], 0, 1, color = ("#CB3C33", 1), linewidth = 0.5) for j in 1:3]
    for j in 1:3
        data = DataFrame(x = df_rg_bi[:, 8 + j], y = df_rg_mul[:, 8 + j])
        model = lm(@formula(y ~ x), data)
        ablines!(axs5[j], coef(model)[1], coef(model)[2], color = :gold, linewidth = 0.5)
    end
    ...
    Label(g5[0, 1:3], text = rich("Comparison of ", rich("r", font = :italic), subscript("g"), " estimates"), fontsize = 8, tellwidth = false)
    Label(g5[1, 1, Top()], rich(rich("r", font = :italic), subscript("g,cis")), fontsize = 6)
    Label(g5[1, 2, Top()], rich(rich("r", font = :italic), subscript("g,trans")), fontsize = 6)
    Label(g5[1, 3, Top()], rich(rich("r", font = :italic), subscript("e")), fontsize = 6)
    Label(g5[0, 1:3, Left()], "f", fontsize = 12, font = "Arial bold", padding = (0, 5, 0, 0), halign = :right)
    Label(g5[2, 1:3], text = "Pairwise bivariate estimates", fontsize = 6)
    Label(g5[1, 0], text = "Multivariate estimates", fontsize = 6, rotation = π / 2, tellheight = false)
    colgap!(g5, 2.5)
    rowgap!(g5, 0)
    ...
    [scatter!(axs6[j], df_rg.rp, df_rg[:, 7 + j], 
        color = (colors[j], 0.5), markersize = 2) for j in 1:3]
    [ablines!(axs6[j], 0, 1, color = ("#CB3C33", 1), linewidth = 0.5) for j in 1:3]
    for j in 1:3
        data = DataFrame(x = df_rg.rp, y = df_rg[:, 7 + j])
        model = lm(@formula(y ~ x), data)
        ablines!(axs6[j], coef(model)[1], coef(model)[2], color = :gold, linewidth = 0.5)
    end
    ...
    Label(g6[0, 1:3], text = "Comparison with phenotypic correlation", fontsize = 8, tellwidth = false)
    Label(g6[1, 1, Top()], rich(rich("r", font = :italic), subscript("g,cis")), fontsize = 6)
    Label(g6[1, 2, Top()], rich(rich("r", font = :italic), subscript("g,trans")), fontsize = 6)
    Label(g6[1, 3, Top()], rich(rich("r", font = :italic), subscript("e")), fontsize = 6)
    Label(g6[0, 1:3, Left()], "f", fontsize = 12, font = "Arial bold", padding = (0, 5, 0, 0), halign = :right)
    Label(g6[2, 1:3], text = rich(rich("r", font = :italic), subscript("p")), fontsize = 6)
    Label(g6[1, 0], text = "Multivariate estimates", fontsize = 6, rotation = π / 2, tellheight = false)
    colgap!(g6, 2.5)
    rowgap!(g6, 0)
    # resize_to_layout!(f)
    save("figures/figure2.pdf", f, pt_per_unit = 1)
end
...
let
    ...
    Label(f[1, 1:3, Top()], "Comparison of variance components estimates", fontsize = 8)
    ...
    [ablines!(axs[1, j], 0, 1, color = ("#CB3C33", 1), linewidth = 0.5) for j in 1:3]
    ...
    [ablines!(axs[2, j], 0, 1, color = ("#CB3C33", 1), linewidth = 0.5) for j in 1:3]
    ...
    rowgap!(f.layout, 1, 1)
    resize_to_layout!(f)
    save("figures/remlvsmle_vc.png", f, px_per_unit = 4)
end
...
let
    ...
    [hidespines!(axs_title[j]) for j in 1:3]
    [hidedecorations!(axs_title[j]) for j in 1:3]
    Label(f[1, 1:3, Top()], "Comparison of heritability estimates", fontsize = 8)
    rowsize!(f.layout, 1, 0.1)
...
let
    ...
    [hidespines!(axs_title[j]) for j in 1:3]
    [hidedecorations!(axs_title[j]) for j in 1:3]
    Label(f[1, 1:3, Top()], "Comparison of heritability estimates for genes with a single isoform", fontsize = 8)
    rowsize!(f.layout, 1, 0.1)
    Label(f[1, 1, Bottom()], "Cis-heritability", fontsize = 6)
    Label(f[1, 2, Bottom()], "Trans-heritability", fontsize = 6)
    Label(f[1, 3, Bottom()], "Total heritability", fontsize = 6)
    ...
    Label(f[3, 1:3], text = "Gene-level estimates", fontsize = 6)
    Label(f[2, 0], text = "Isoform-level estimates", fontsize = 6, rotation = pi / 2, tellheight = false)
    colgap!(f.layout, 5)
    rowgap!(f.layout, 1)
    resize_to_layout!(f)
